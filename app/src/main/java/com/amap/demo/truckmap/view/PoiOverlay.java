...
public class PoiOverlay {
	 ...
	 * @since V2.1.0
	 */
	public PoiOverlay(AMap amap, List<PoiItem> pois , int type) {
		mAMap = amap;
		mPois = pois;
		mType = type;
	}
	...
	public void addToMap() {
		try{
			for (int i = 0; i < mPois.size(); i++) {
				...
				marker.setObject(i);
				Log.d("qyd", "addToMap " + mPois.get(i).getTypeCode());
	...
	protected BitmapDescriptor getBitmapDescriptor(int index) {
		if (mType == 1) {
			return BitmapDescriptorFactory.fromResource(R.drawable.gas);
		} else {
